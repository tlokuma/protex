...
<script>  
                       $(".help" ).click(function(e) {
                        ...
                        Swal.fire({
                        title: 'Menu de Ajuda',
                        text: "Bem vindo ao sistema PROTEX, você esta no CADASTRO DE NOVO SUBGRUPO, onde você pode cadastrar um novo subgrupo. Através da barra de navegação situada na parte superior do site, você pode realizar lançamentos, gerenciar os seus lançamentos, dar baixas (na seção de gerenciar) cadastrar e gerenciar os seus grupos. Além disso sempre que quiser voltar para a página inicial você pode clicar em Página Inicial ou em PROTEX!",
  ...
  <div class= "titulonovogrupo"><p><b>Cadastro de Novo Subgrupo</b></p></div>
    <div class="novogrupoform">
        <form action="subgruposprocessa.php" method="POST">
        <div class="form-group col-md-12">
        <label for="selectgrupo">Grupo Principal</label>
        <select  name="grupoprinc" id="grupoprinc" class="form-control" required>
        <?php
              ...
              if($result != 0){
                foreach($resultado_grupos as $resug ){?>
                  ...
                  <option value="<?php echo $resug['id_grupo']; ?>"><?php
        ...
        <div class="form-group col-md-12">
            <label for="novogrupo">Novo Subgrupo</label>
            <input type="text" class="form-control" id="novogrupo" minlength="3" maxlength="30" name="novogrupo" placeholder="Nome do Subgrupo" required>
          </div>
          <button type="submit" name="submit" id="cadastrar" class="btn btn-success posicionabotao">Gravar</button>
        </form>
    </div>
