    ...
    if(!empty($_GET['id'])){
        ...
        if($result -> num_rows > 0){
          while($user_data = mysqli_fetch_assoc($result)){
            $nome = $user_data['nome'];
            $idgrupo = $user_data['id_grupo'];

            $sqlGrupo = "SELECT nome, id_grupo FROM grupo WHERE id_grupo=$idgrupo;";
            $resultgrupo = $conexao->query($sqlGrupo);    
            $usergrupo = mysqli_fetch_assoc($resultgrupo);
            $nomegrupo = $usergrupo['nome'];
...
<script>  
                       $(".help" ).click(function(e) {
                        ...
                        Swal.fire({
                        title: 'Menu de Ajuda',
                        text: "Bem vindo ao sistema PROTEX, você esta no EDITAR SUBGRUPO, onde você pode editar um subgrupo previamente cadastrado. Através da barra de navegação situada na parte superior do site, você pode realizar lançamentos, gerenciar os seus lançamentos, dar baixas (na seção de gerenciar) cadastrar e gerenciar os seus grupos. Além disso sempre que quiser voltar para a página inicial você pode clicar em Página Inicial ou em PROTEX!",
 ...
 <div class= "titulonovogrupo"><p><b>Editar Subgrupo</b></p></div>
    <div class="editgrupoform">
        <form action="editsubgruposprocessa.php" method="POST">
        <div class="form-group col-md-12">
              <label for="gruporeceber">Grupo</label>
              <select name="idgrupo" id="idgrupo" class="form-control" value="<?php echo $nome ?>" required>
                <?php
                ...
                <?php
                foreach($resultado_grupos as $row_grupos){
                 ?>
                <option value="<?php echo $row_grupos['id_grupo']; ?>" <?php if($row_grupos['id_grupo'] == $idgrupo) echo"selected"; ?>><?php echo $row_grupos['nome']; ?>
                  </option> <?php 
                }
        ...
        <div class="form-group col-md-12">
            <label for="novogrupo">Novo nome do SubGrupo</label>
            <input type="text" minlength="5" maxlength ="20" name="novosubgrupo" id="novosubgrupo" class="form-control" value="<?php echo $nome ?>" required>
          </div>
          <input type ="hidden" name="idgrupoantigo" id="idgrupoantigo" value="<?php echo $idgrupo?>">
