...
while ($user_data = mysqli_fetch_assoc($pagoounao))
{
  ...
  if ($status>0){
    print "<script> alert('Você não pode editar uma registro que já tem parcelas pagas'); </script>";
    print "<script> location.href='lancamentosreceber.php'; </script>";
  }
}
...
<script>  
                       $(".help" ).click(function(e) {
                        ...
                        Swal.fire({
                        title: 'Menu de Ajuda',
                        text: "Bem vindo ao sistema PROTEX, você esta no EDITAR DADOS CONTAS A RECEBER, onde você pode alterar os dados do resgistro de contas a receber que você selecionou em gerenciar lançamentos contas a receber. Através da barra de navegação situada na parte superior do site, você pode realizar lançamentos, gerenciar os seus lançamentos, dar baixas (na seção de gerenciar) cadastrar e gerenciar os seus grupos. Além disso sempre que quiser voltar para a página inicial você pode clicar em Página Inicial ou em PROTEX!",
    ...
    <form action="saveEditReceber.php" method="POST">
          <div class = "titulolancamento"><p><b>Editar Dados - Contas a Receber</b></p></div>
          <div class="form-group">
            <label for="clientereceber">Nome do Cliente</label>
            <input type="text" class="form-control" id="clientereber" name="clientereceber" value="<?php echo $cliente ?>" required>
          </div>
          <div class="form-row">
          <div class="form-group col-md-6">
              ...
              <select id="gruporeceber" name="gruporeceber" class="form-control" required>
                ...
                { ?>
                <option><?php echo $row_grupos['nome']; ?>
                </option> <?php } ?>
              </select>
            </div>
            <div class="form-group col-md-6">
              ...
              <select id="subgruporeceber" name="subgruporeceber" class="form-control" required>
                <option selected><?php echo $subgrupo ?></option>
                <?php
                ...
                { ?>
                <option><?php echo $row_subgrupos['nome']; ?>
                </option> <?php } ?>
              </select>
            </div>
          </div>
          ...
          <div class="form-group col-md-2">
            ...
            <input type="date" class="form-control" id="vencimentoreceber" name="vencimentoreceber" value="<?php 
                                    $mostravencimentoaux = $vencimento;
                                    $mostravencimento = date('Y-m-d', $mostravencimentoaux);
                                    echo $mostravencimento;
            ?>" required>
          </div>
          <div class="form-group col-md-2">
            <label for="valorrceber">Valor</label>
            <input type="number" class="form-control" id="valorreceber" name="valorreceber" value="<?php echo $valor ?>" required>
          </div>
          <div class="form-group col-md-2">
            <label for="numparcelasreceber">Número de Parcelas</label>
            <input type="number" class="form-control" id="numparcelasreceber" name="numparcelasreceber" value="<?php echo $numparcelas ?>" required>
          </div>
          <div class="form-group col-md-6">
            <label for="historicoreceber">Histórico</label>
            <input type="text" class="form-control" id="historicoreceber" name="historicoreceber" value="<?php echo $historico ?>" required>
          </div>
          </div>
          <div>
          <input type="hidden" name="id" value="<?php echo $id ?>">
          <input type="hidden" name="valorparcelareceber" value="<?php echo $valorparcela ?>">
          </div>
          <div class="form-group">
          <label for="historicolongoreceber">Histórico Longo</label>
          <input type="text" class="form-control" id="historicolongoreceber" name="historicolongoreceber" value="<?php echo $historicolongo ?>" required>
          </div>
          <button type="submit" name="update" id="update" class="btn btn-success">Gravar Edição</button>
    </form>
    </div> 
